...
- `notification/` — user notifications (`Notification` entity, receiver-centric, FK-free snapshots of actor/trip/feedback). `TripFeedbackService.create` calls `NotificationService.notifyFeedback` with a `FeedbackNotificationCommand` so `notification/` never depends on `trip/`. Stored first, then pushed **after commit** over SSE (`GET /api/notifications/stream`, `NotificationSseRegistry` keeps per-user emitters in memory with a 25s heartbeat; single-instance only). List/unread-count/read endpoints under `/api/notifications`. Trip deletion and account deletion remove the related rows.
